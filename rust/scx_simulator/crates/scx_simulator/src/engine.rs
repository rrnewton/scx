...
/// Core dispatch worker body shared by cooperative, preemptive, and Frida
...
/// Create a per-thread PMU timer for preemptive interleaving.
///
/// Returns `(None, -1)` if `cooperative_only` is true or if the PMU is
/// unavailable (VMs, containers).
fn setup_pmu_timer(cooperative_only: bool) -> (Option<perf::RbcTimer>, std::os::unix::io::RawFd) {
    use crate::preempt;
    ...
    let timer_fd = match &timer {
        Some(t) => {
            ...
            match t.set_signal_delivery(tid, preempt::PREEMPT_SIGNAL) {
                Ok(()) => t.raw_fd(),
                Err(e) => {
                    tracing::warn!("PMU signal delivery setup failed: {e}");
                    -1
                }
            }
        }
        ...
        std::thread::scope(|s| {
            ...
            for (i, &cpu) in dispatch_cpus.iter().enumerate() {
                ...
                s.spawn(move || {
                    ...
                    let (timer, timer_fd) = setup_pmu_timer(cooperative_only);
