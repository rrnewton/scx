...
fn test_replay_determinism() {
    use scx_simulator::{drain_preemption_records, enable_preemption_collection, PreemptionTrace};
    ...
    let replay_trace = PreemptionTrace::from_records(&records, num_workers);
    eprintln!(
        "Recorded {} preemption points across {} workers",
        replay_trace.total_preemptions(),
    ...
    assert_eq!(
        trace1.events().len(),
        trace2.events().len(),
        "replay traces have different lengths: {} vs {}",
        trace1.events().len(),
        trace2.events().len()
    );
    ...
    assert_eq!(
        mismatches,
        0,
        "replay mode: {} trace event mismatches out of {} events",
        mismatches,
        trace1.events().len()
    );
}
